...
def _parse_stdin_context(raw_stdin):
    ...
    try:
        ...
        # Try used_percentage first (API format)
        used_pct = ctx.get("used_percentage")
        if used_pct is not None:
            result["context_pct"] = float(used_pct)

        # Try total_input_tokens (legacy format)
        input_tok = ctx.get("total_input_tokens")
        output_tok = ctx.get("total_output_tokens")

        # Fall back to current_usage (Claude Code standard format)
        if input_tok is None:
            ...
            if inp or cr or cw or out:
                input_tok = inp + cr + cw
                output_tok = out

        if input_tok is not None and ctx_size:
...
def build_status_line(usage, plan, config=None, stdin_ctx=None, cache_age=None):
    ...
    ctx_line2 = None  # populated when context_format == "full" to render on line 2
    if stdin_ctx and show.get("context", True):
        ...
        if ctx_pct is not None:
            ...
            if ctx_fmt == "tokens":
                ...
                remain = max(0, ctx_limit - ctx_used)
                remain_pct = max(0, 100 - ctx_pct)
                _fk = lambda n: (f"{round(n/1000)}k" if n >= 1000 else str(int(n)))
                pct_label = f"{ctx_pct:.0f}% {_fk(ctx_used)} \u2022 {remain_pct:.0f}% {_fk(remain)} left"
            else:
            ...
            _cx = _pri("context")
            if ctx_fmt == "full":
                # Render context on line 2 to avoid truncation on narrow terminals
                warn_prefix = f"{ctx_warning_label} " if ctx_warning_label else "Context "
                ctx_line2 = f"{warn_prefix}{ctx_bar} {pct_label}{ctx_warning_suffix}"
            elif layout == "compact":
    ...
    # Append context line 2 (used when context_format == "full")
    if ctx_line2:
        line = line + "\n" + ctx_line2
...
def _truncate_single(line, max_visible):
    """Clip a single line (no newlines) to max_visible characters, preserving ANSI codes."""
    visible_count = 0
    cut = None
    i = 0
    while i < len(line):
...
def _truncate_line(line, config):
    ...
    Handles multi-line output (newline-separated) by truncating each line
    independently.
    """
    try:
        term_width = _detect_terminal_width() or shutil.get_terminal_size((120, 24)).columns
        ...
        if "\n" in line:
            segments = line.split("\n")
            line = "\n".join(_truncate_single(seg, max_visible) for seg in segments)
        else:
            line = _truncate_single(line, max_visible)
    except Exception:
        pass
    return line
...
def main():
    ...
    if "--context-format" in args:
        ...
        if idx + 1 < len(args):
            val = args[idx + 1].lower()
            if val not in ("percent", "tokens", "full"):
                utf8_print(f"Unknown format: {_sanitize(val)}  (use percent, tokens, or full)")
                return
            config = load_config()
            ...
            utf8_print("Usage: --context-format percent|tokens|full")
        return
